...
# Fitting über vorgegebene Startwerte
fit <- nls(velo ~ (V_max * sub) / (K_m + sub), 
           start = c(V_max = 15 , K_m = .5))
coef(fit)
# Fitting über ein selfstart-Modell
fit2 <- nls(velo ~ SSmicmen(sub, Vm, K))
coef(fit2)

tibble(coef(fit), coef(fit2)) %>% 
  print() %>% 
  pander()
...
ggplot2::ggplot(mapping = ggplot2::aes(x = sub, y = velo))+
  ...
  ggplot2::geom_text(aes(0,coef(fit)[[1]],label = round(coef(fit)[[1]], digits = 3), vjust = 1.4), hjust = .3 )+ # Vmax aus coefficents eintragen
  ggplot2::geom_hline(yintercept =( coef(fit)[[1]])/2,)+ # Vmax/2 aus coefficents
  ggplot2::geom_text(aes(0,(coef(fit)[[1]])/2,label = "Vmax/2", vjust = 1.4), hjust = .3)+ # Vmax/2 aus coefficents eintragen
  ggplot2::geom_vline(xintercept = coef(fit)[[2]])+ # Km aus coefficents
  ggplot2::geom_text(aes(0,coef(fit)[[2]],label = round(coef(fit)[[2]], digits = 3), vjust = 1.41), hjust = -2.3) # Km aus coefficents eintragen 
```
